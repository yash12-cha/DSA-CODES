...
def order_agnostic_binary_search(input_array, target):
    """
    Perform binary search on an array of unknown order (sorted or reverse sorted).
    
    Args:
    - input_array (list of int): The array in which to search.
    - target (int): The element to search for.
    ...
    """
    left = 0  # index of the leftmost element
    right = len(input_array) - 1  # index of the rightmost element

    # Determine if the array is sorted in ascending or descending order
    if input_array[left] < input_array[right]:
        result = binary_search(input_array, target)  # perform binary search on a sorted array
    else:
        result = reverse_binary_search(input_array, target)  # perform binary search on a reverse sorted array
